...
class HaConfig(object):
    def __init__(self, base_url: str, token: str, tts_config: TtsConfigFromEnv, webhook_id: str, cache_dir: Optional[str]):
...
async def print_tts_providers(ha_config: HaConfig) -> None:
    parsed = urlparse(ha_config.base_url)
    scheme = 'wss' if parsed.scheme == 'https' else 'ws'
    ws_url = urlunparse((scheme, parsed.netloc, '/api/websocket', '', '', ''))
    async with websockets.connect(ws_url) as websocket:
        ...
        if not has_language:
            ...
            return
        log(None, f"TTS provider {engine_from_config} was found and supports language {ha_config.tts_config['language']}")
        await websocket.send(json.dumps({
            "id": 2,
